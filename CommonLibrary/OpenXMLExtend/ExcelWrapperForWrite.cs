...
namespace CommonLibraryStandard
{
  ...
  public partial class ExcelWrapper : IExcelWrapper
  {
	...
	public async Task WriteToXlsxFileAsync(WriteXlsxFileParams wxfPrms)
	{
	  ...
	  using FileStream fs = new FileStream(wxfPrms.FileName,
	...
	{
	  writeParams = new WriteXlsxFileParams(wxfPrms);
	...
	private Cell CreateCell(SheetCellItem itm, uint row, SharedStringTablePart ssPart)
	{
	  ...
	  switch (itm.DataType)
	  {
		case DataTypes.Number:
		  result = new Cell() { DataType = CellValues.Number, CellReference = col + row.ToString(), CellValue = new CellValue(itm.Data) };
		  break;
		case DataTypes.String:
		  result = new Cell() { DataType = CellValues.InlineString, CellReference = col + row.ToString() };
		...
		case DataTypes.SharedString:
		  ...
		  result = new Cell() { DataType = CellValues.SharedString, CellReference = col + row.ToString(), CellValue = new CellValue(index.ToString()) };
		  break;
	  }
	...
	private Fonts fonts = new Fonts(new Font(new FontStyle { FontSize = 10 }.ToOpenXmlFont()));
	private Fills fills = new Fills(new Fill(new FillStyle().ToOpenXmlFill()));
	private Borders borders = new Borders(new BorderStyle().ToOpenXmlBorder());
	private CellFormats cellFormats = new CellFormats(new CellFormat() { FillId = 0, FontId = 0, BorderId = 0 });
	...
	private List<(int sheetindex, uint headformatindex, uint dataformatindex)> GenerateSheetStyleForSheets()
	{
	  ...
	  {
		result = new List<(int sheetindex, uint headformatindex, uint dataformatindex)>();

		index_fontstyle = new List<(uint index, FontStyle fontstyle)>();
		index_fillstyle = new List<(uint index, FillStyle fillstyle)>();
		index_borderstyle = new List<(uint index, BorderStyle borderstyle)>();
		index_alignstyle = new List<(uint index, AlignmentStyle alignstyle)>();
		index_styleitem = new List<(uint index, StyleItem styleitem)>();
		for (int i = 0; i < writeParams.SheetData.Count(); i++)
		{
		  if (writeParams.SheetData.ElementAt(i) is ExlsSheetData sheet)
		  {
			(int sheetindex, uint headformatindex, uint dataformatindex) ri = (i, 0, 0);
	...
	private List<(int sheetindex, uint rowindex, uint colindex, uint formatindex)> GenerateSheetStyleForCells()
	{
	  ...
	  {
		result = new List<(int sheetindex, uint rowindex, uint colindex, uint formatindex)>();
			  ...
			  if (sheet.SheetRows[ri - 1]?.RowCells.Any(x => x.CellStyle != null) == true)
			  {
				for (var ci = 1; ci <= sheet.SheetRows[ri - 1].RowCells.Count; ci++)
				{
				  ...
				  if (theCellStyle != null)
				  {
					...
					result.Add((i, (uint)ri, (uint)ci, fi));
				  }
				}
			  }
			}
