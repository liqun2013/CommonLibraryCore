  ...
  public class ExlsSheetData
  {
	...
	public List<T> ToList<T>() where T : new()
	{
	  ...
	  if (SheetRows?.Any() == true)
	  {
		...
		List<(uint order, PropertyInfo property)> lstOrderAndProperty = new List<(uint order, PropertyInfo property)>();
	...
	public ExlsSheetData ToSheetdata<T>(IEnumerable<T> data, bool addHeadRow)
	{
	  if (data?.Any() == true)
	  {
		...
		List<(uint DisplayOrder, string ColName, string PropertyName, uint ColWidth)> rowDataAtts = new List<(uint DisplayOrder, string ColName, string PropertyName, uint ColWidth)>();
		SheetRows = new List<SheetRowItem>();
		...
		if (addHeadRow)
		{
		  ...
		  foreach ((_, string ColName, _, uint ColWidth) in rowDataAtts.OrderBy(x => x.DisplayOrder))
			headRow.RowCells.Add(new SheetCellItem() { Data = ColName, ColIndex = colIndex++, DataType = DataTypes.String, CustWidth = ColWidth });
